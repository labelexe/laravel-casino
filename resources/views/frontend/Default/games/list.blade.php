...
@section('content')
		...
		<div class="container">
			<div class="footer__block">
				...
				<div class="footer__center">
					<div class="footer__info" style="max-width:100%;">
						<span class="account-id"><em>ID</em> {{ Auth::user()->username }}</span>
						<a href="#modal-pin" class="footer__btn modal-btn btn" data-name="modal-pin">пополнить</a>
						<a href="#modal-payout" class="footer__btn modal-btn btn" data-name="modal-payout">вывести</a>
						<a href="#modal-pay-history" id="pay-history" class="footer__btn modal-btn btn" data-name="modal-pay-history">история</a>
	...
	<div class="modal modal-pin">
		<div class="modal__block">
			<h3 class="modal__title">Enter replenishment amount</h3>
			<!--<p class="modal__text">Enter PIN - code to replenish your account<span id="error" style="color: red;"></span></p>-->
			<div class="input__group">
				<input type="text" style="margin-bottom:10px;" placeholder="сумма пополнения в рублях" id="inputAmount">
			</div>
	...
	<div class="modal modal-payout">
		<div class="modal__block">
			<h3 class="modal__title">Enter payout amount</h3>
			<!--<p class="modal__text">Enter PIN - code to replenish your account<span id="error" style="color: red;"></span></p>-->
			<div class="input__group">
				<input type="text" style="margin-bottom:10px;" placeholder="сумма вывода в рублях" id="outputAmount">
			</div>
			<div class="input__group">
				<input type="text" style="margin-bottom:10px;" placeholder="телефон для связи" id="outputPhone">
				<input type="text" style="margin-bottom:10px;" placeholder="номер карты для вывода" id="outputCard">
			</div>

			<button type="submit" class="btn" id="send_payout">OK</button>
		</div>
		<span class="close-btn">
				<img src="/frontend/Default/img/_src/close.svg" alt="">
			</span>
	...
	<div class="modal modal-pay-history" style="max-height:350px !important; min-height:350px !important;">
		<div class="modal__block">
			<h3 class="modal__title">Your pay history</h3>
			<div id="history-block" style="max-height:300px !important;">
	...
	<script type="text/javascript">
        ...
        $('body').on('click', '#send_payout', function(event) {
            var outputAmount = $('#outputAmount').val();
            var phone = $('#outputPhone').val();
            var card = $('#outputCard').val();

            $.ajax({
                url: "{{ route('frontend.profile.payout') }}",
                type: "GET",
                data: {
        ...
        $('body').on('click', '#pay-history', function(event) {
            //var outputAmount = $('#outputAmount').val();

            $.ajax({
                url: "{{ route('frontend.profile.pay_history') }}",
                type: "GET",
                data: {},
                dataType: "json",
                success: function(data) {
                    ...
                    if( data.success ) {
                        $('#history-block').css({'overflow':'scroll'});
						for(var i=0; i!=data.history_data.length; i++) {
                        	$('#history-block').append('<p style="text-align:left;" class="modal__text"><span>'+data.history_data[i].date+' '+data.history_data[i].type+' '+data.history_data[i].summ+' рублей</span></p>');
						}
                    }
                }
            });
