...
function CTSlotApp() {
    ...
    function It() {
        if (QAtrace = "\nSTART\n", ht = 0, oe = 0, (mt + currSession.gameplay_mintime < (new Date).getTime() || mt < 1) && ("gameInit" === state || "prevWinDisplayLoop" === state)) {
            if (En(), Math.floor(m / currDenom.cents) < currTotalBet && !freeround_limit) switch (autoplay) {
                case 1:
                    Dn(), Et();
                case 0:
                    return void Bn()
...
freeround_win=parseFloat(freeround_win);
    ...
    function In() {
        startWait = 0, $(st).off("touchstart mousedown", On), (Math.floor(m / currDenom.cents) > 0 || freeround_limit) && (gs(In), ct ? Qn("play") : It())
    }
...
var t = e % 100,
    ...
    function zn() {
        if (we) var e = Math.floor(m / currDenom.cents),
            t = e > 0 ? e : "";
        else t = Gn(m);
        ms("#frameCredit", t)
    }
 ...
 $(s_oMain).trigger("exitGame");
    ...
    this.subgameStarted = function() {
        ...
        if (recovery) {
            ...
            for (extraBet = gameResume.extra_bet, playerAccount = gameResume.account_status_after_bet, currBetPerLine = parseInt(gameResume.line_bet), currNumLines = parseInt(gameResume.lines), state = "subgameFinished", n = 0; n < denomNum; n++)

			
                if (ve[0].id == gameResume.game_denom) {
                    //denomIndex = gameResume.denomIndex;$("#frameTotalBet").find(".string1").html(fo(parseInt(currNumLines * currBetPerLine / gameConfig.lines_per_credit)));
                   // break;
                } 
