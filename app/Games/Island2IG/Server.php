...
namespace App\Games\Island2IG;

use App\Games\Island2IG\SlotSettings as SlotSettings;
...
/*            if( config('LicenseDK.APL_INCLUDE_KEY_CONFIG') != 'wi9qydosuimsnls5zoe5q298evkhim0ughx1w16qybs2fhlcpn' )
        ...
        else if( $_obf_0D221D1040101E0C18152D38350A220B2431190A3E1822['action'] == 'risk' && $slotSettings->GetGameData('Island2IGWin') <= 0 )
        {
        ...
        switch( $_obf_0D221D1040101E0C18152D38350A220B2431190A3E1822['action'] )
        {
            case 'init':
                ...
                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","slotViewState":"' . $slotSettings->slotViewState . '","slotKeyConfig":' . $slotSettings->slotKeyConfig . ',"BetArray":[' . implode(',', $slotSettings->Bet) . '],"Credit":"' . floor($balance) . '","Currency":"' . $slotSettings->slotCurrency . '","Denomination":"' . sprintf('%01.2f', $slotSettings->CurrentDenom) . '","FuseBet":' . $slotSettings->fuseBet . ',"HelpCoef":[[0,0,0],[200,1000,5000],[100,500,2000],[30,100,500],[20,50,200],[10,30,100],[5,10,50],[3,5,20],[2,3,10]],"LastBet":' . $bet . ',"LastFuse":false,"LastLines":' . $lines . ',"MaxWin":675000,"RawCredit":1000,"Reels":[' . $_obf_0D282A3E130518100418230D042B0A2B400930193F3232 . '],"Win":0,"args":{},"cmd":"init","time":"20181111T111512"}';
                break;
            case 'status':
                if( !$slotSettings->HasGameData('Island2IGLines') || !$slotSettings->HasGameData('Island2IGBet') )
                {
                    $slotSettings->SetGameData('Island2IGLines', 1);
                    $slotSettings->SetGameData('Island2IGBet', 1);
                }
                $balance = $slotSettings->GetBalance();
                if( $slotSettings->GetGameData('Island2IGWin') > 0 )
                {
                    $balance = $balance - $slotSettings->GetGameData('Island2IGWin');
                }
                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","BetArray":[' . implode(',', $slotSettings->Bet) . '],"Credit":"' . floor($balance) . '","Currency":"' . $slotSettings->slotCurrency . '","Denomination":"' . sprintf('%01.2f', $slotSettings->CurrentDenom) . '","FuseBet":' . $slotSettings->fuseBet . ',"HelpCoef":[[0,0,0],[200,1000,5000],[100,500,2000],[30,100,500],[20,50,200],[10,30,100],[5,10,50],[3,5,20],[2,3,10]],"LastBet":' . $slotSettings->GetGameData('Island2IGBet') . ',"LastFuse":false,"LastLines":' . $slotSettings->GetGameData('Island2IGLines') . ',"MaxWin":675000,"RawCredit":"' . $balance . '","Reels":[[7,1,5,3,8],[5,3,6,7,7],[6,8,7,8,1]],"Win":0,"args":{},"cmd":"status","time":"20181111T111512"}';
                break;
            case 'start':
                $balance = $slotSettings->GetBalance();
                ...
                $slotSettings->SetGameData('Island2IGReels', $_obf_0D282A3E130518100418230D042B0A2B400930193F3232);
                $slotSettings->SetGameData('Island2IGWin', $totalWin);
                $slotSettings->SetGameData('Island2IGRisk', 1);
                $slotSettings->SetGameData('Island2IGLines', $lines);
                $slotSettings->SetGameData('Island2IGBet', $bet);
                $slotSettings->SetGameData('Island2IGDealerCard', $_obf_0D152F2B13101C1F2D320B1C302214192A26071A401E32);
            ...
            case 'finish':
                $balance = $slotSettings->GetBalance();
                $slotSettings->SetGameData('Island2IGWin', 0);
                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","Credit":' . floor($balance) . ',"RawCredit":' . floor($balance) . ',"Reels":[' . $slotSettings->GetGameData('Island2IGReels') . '],"args":{},"cmd":"finish","time":"20181111T171830"}';
                break;
            case 'risk':
