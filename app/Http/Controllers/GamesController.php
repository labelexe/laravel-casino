...
/*            $checked = new \VanguardLTE\Lib\LicenseDK();
        ...
        public function categories(\Illuminate\Http\Request $request)
        {
            ...
            if( $request->action == 'delete_games' && count($games) )
            {
                ...
                foreach( $gameArray as $game )
                {
                    ...
                    \App\Models\StatGame::where('game', $game->name)->delete();
                    \App\Models\Games::where('id', $game->id)->delete();
                }
                return redirect()->route('backend.game.list')->withSuccess(trans('app.games_deleted'));
            }
            if( $request->action == 'stay_games' && count($games) )
            {
                $count = \App\Models\Games::whereNotIn('id', $games)->where('shop_id', auth()->user()->shop_id)->count();
                $pages = ceil($count / 100);
                for( $i = 0; $i < $pages; $i++ )
                {
                    $gameArray = \App\Models\Games::whereNotIn('id', $games)->where('shop_id', auth()->user()->shop_id)->take(100)->get();
                    foreach( $gameArray as $game )
                    {
                        \VanguardLTE\Task::create([
                            'category' => 'game',
        ...
        public function view(\App\Models\User $user, \VanguardLTE\Repositories\Activity\ActivityRepository $activities)
        {
        ...
        {
            $edit = true;
            $game = \App\Models\Games::where('id', $game)->firstOrFail();
            ...
            $jpgs = \App\Models\JPG::where('shop_id', auth()->user()->shop_id)->pluck('name', 'id')->toArray();
        ...
        public function mass(\Illuminate\Http\Request $request)
        {
            ...
            $emptyGame = new \App\Models\Games();
        ...
        public function go(\Illuminate\Http\Request $request, $game)
        {
            ...
            $object = 'App\Games\\'.$game.'\SlotSettings';
            //$object = 'App\\Http\\Controllers\\' . $game . '\SlotSettings';
            //$object = 'App\\Http\\Controllers\\' . $game . '\SlotSettings';
            $slot = new $object($game, $userId);
            $game = \App\Models\Games::where('name', $game)->first();
            return view('frontend.games.list.' . $game->name, compact('slot', 'game'));
        }
        public function server(\Illuminate\Http\Request $request, $game)
        {
            //$object = 'App\Games\AmericanFootball\Server';
            $object = 'App\Games\\' . $game . '\Server';
            // $object = '\VanguardLTE\Games\\' . $game . '\Server';
            $server = new $object();
            //dd('resp');
            $result = $server->get($request, $game);
            //dd($result);
            return $result;
        }
        ...
        {
            $request->validate([
                'gamebank' => 'sometimes|required|in:' . implode(',', \App\Models\Games::$values['gamebank']),
                'rezerv' => 'sometimes|required|in:' . implode(',', \App\Models\Games::$values['rezerv']),
                'cask' => 'sometimes|required|in:' . implode(',', \App\Models\Games::$values['cask'])
            ]);
            $data = $request->only([
                'name',
                'title',
            ...
            $gameData = \App\Models\Games::find($game);
            ...
            if( count($data) > 0 )
            {
                $win = \App\Models\GameWin::where('game_id', $gameData->id)->first();
                foreach( [
                    1,
                    3,
                    5,
        ...
        public function delete(\App\Models\Games $game)
        {
            if( !in_array($game->shop_id, auth()->user()->availableShops()) )
            {
                return redirect()->back('backend.game.list')->withErrors([trans('app.wrong_shop')]);
            ...
            event(new \VanguardLTE\Events\Game\DeleteGame($game));
            \App\Models\GameWin::where('game_id', $game->id)->delete();
            \VanguardLTE\GameCategory::where('game_id', $game->id)->delete();
            \App\Models\StatGame::where('game', $game->name)->delete();
            \App\Models\GameLog::where('game_id', $game->id)->delete();
            \App\Models\Games::where('id', $game->id)->delete();
        ...
        public function gamebanks_add(\Illuminate\Http\Request $request)
        {
            ...
            if( !$request->type || !in_array($request->type, \App\Models\Games::$values['gamebank']) )
            {
...
/*        public function security()
         ...
         * @OA\Get(
         *     path="/api/games/all",
         *     summary="Получение списка игр из БД",
         *     tags={"Games"},
         *
         *     @OA\Parameter(
         *         name="page",
        ...
        public function getList(\Illuminate\Http\Request $request)
        {
            ...
            $games = Games::all()->skip($skip)->take($limit);
            $games->load('logo');
        ...
        public function getOneById($id) {
            $person = Games::where('id', $id)->firstOrFail();
